...
from gnn_tracking_prev.utils.math import zero_division_gives_nan
from gnn_tracking_prev.utils.nomenclature import denote_pt
from gnn_tracking_prev.utils.signature import tolerate_additional_kwargs
...
def tracking_metric_df(h_df: pd.DataFrame, predicted_count_thld=3) -> pd.DataFrame:
    ...
    acorn_matching_fraction = 0.5 + 1e-12

    # one-way
    c_df["one_way_reconstructed"] = c_df["maj_pid_frac"] >= acorn_matching_fraction
    c_df["one_way_matched"] = c_df["maj_frac"] >= acorn_matching_fraction

    # two-way
    c_df["two_way_reconstructed"] = (
        (c_df["maj_pid_frac"] >= acorn_matching_fraction)
        & (c_df["maj_frac"] >= acorn_matching_fraction)
    )
    c_df["two_way_matched"] = c_df["two_way_reconstructed"]

    # ATLAS
    c_df["ATLAS_matched"] = c_df["maj_frac"] >= acorn_matching_fraction
    c_df["ATLAS_reconstructed"] = c_df["maj_pid"].isin(
        c_df["maj_pid"][c_df["ATLAS_matched"]]
    )
    return c_df
...
def count_tracking_metrics(
    ...
    n_one_way_reconstructed = sum(c_df["one_way_reconstructed"][c_mask])
    n_one_way_matched = sum(c_df["one_way_matched"][c_mask])
    n_two_way_matched = sum(c_df["two_way_reconstructed"][c_mask])
    n_two_way_reconstructed = sum(c_df["two_way_reconstructed"][c_mask])
    n_ATLAS_matched = sum(c_df["ATLAS_matched"][c_mask])
    n_ATLAS_reconstructed = sum(c_df["ATLAS_reconstructed"][c_mask])

    matched_tracks_mask = c_mask & c_df["ATLAS_matched"]
    n_matched_tracks = int(matched_tracks_mask.sum())
    if len(valid_pids) > 0:
        matched_pids = c_df.loc[matched_tracks_mask, "maj_pid"]
    ...
    n_dup_reconstructed_particles = n_matched_tracks - n_reconstructed_particles
    ...
    r: TrackingMetrics = {
        ...
        "one_way_reconstructed": n_one_way_reconstructed,
        "one_way_matched": n_one_way_matched,
        "two_way_reconstructed": n_two_way_reconstructed,
        "two_way_matched": n_two_way_matched,
        "ATLAS_reconstructed": n_ATLAS_reconstructed,
        "ATLAS_matched": n_ATLAS_matched,
        "technical_efficiency": zero_division_gives_nan(
            n_reconstructed_particles, n_particles
        ),
        "fake_rate": zero_division_gives_nan(
            n_clusters - n_matched_tracks, n_clusters
        ),
        "dup_rate": zero_division_gives_nan(
            n_dup_reconstructed_particles, n_reconstructed_particles
        ),
    }
    return r
    ...
    *,
    ...
    for pt in pt_thlds:
        ...
        h_mask = (
            ...
            & (h_df["pid_n_hits"] >= min_track_length)
        )
