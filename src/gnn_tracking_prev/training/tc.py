...
from gnn_tracking_prev.metrics.losses import BackgroundLoss, PotentialLoss
from gnn_tracking_prev.postprocessing.dbscanscanner import DBSCANHyperParamScanner
from gnn_tracking_prev.training.base import TrackingModule
from gnn_tracking_prev.utils.dictionaries import add_key_suffix, to_floats
from gnn_tracking_prev.utils.lightning import obj_from_or_to_hparams
from gnn_tracking_prev.utils.oom import tolerate_some_oom_errors
...
def modi_all(d):
    for k, v in d.items():
        if isinstance(v, dict):
            ...
            all_v = v.split(".")
            if all_v[0] == "gnn_tracking":
                all_v[0] = "gnn_tracking_prev"
                d[k] = ".".join(all_v)
...
class TCModule(TrackingModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        modi_all(potential_loss)
        modi_all(background_loss)
        modi_all(cluster_scanner)
