...
class TrackingTransform(BaseTransform):
    def forward(self, data):
        ...
        if hasattr(data, "hit_id") and data.hit_id is not None:
            ...
            # Preserve a stable per-hit identifier for downstream alignment logic.
        ...
        # full_masks = torch.stack(full_masks, dim=0).T.unsqueeze(-1)
        # labels = torch.zeros(len(good_pid), dtype=torch.long)
        # data.targets = [{"labels": labels, "masks": masks, "y_true": (data.pt >= 0.9) & (data.reconstructable == 1)}]
        # data.full_masks = [{"full": full_masks}]
        ...
        # condition = (data.pt >= 0.9) & (data.reconstructable == 1)
        ...
        # if condition.shape[0] == data.targets[0]['masks'].shape[1]:
        #     data.targets[0]['masks'] = data.targets[0]['masks'][:, condition]
        return data
...
class Tracking(InMemoryDataset):
    def __init__(self, root, dataset_name, debug=False, **kwargs):
        assert dataset_name in ["tracking-6k", "tracking-60k", "trackml-large"]
        ...
        self.n_sectors = 1 if dataset_name in ["tracking-60k", "trackml-large"] else 10
        self.debug = debug
        ...
        self.data, self.slices, self.idx_split = torch.load(self.processed_paths[0], weights_only=False)
        self.idx_split = get_new_idx_split(self)
        # self.x_dim = self._data.x.shape[1]  # 14 + 1 = 15
        self.x_dim = 15
        self.coords_dim = 2 + 4
    ...
    def raw_file_names(self):
        return ["data21575_s0.pt"] if self.dataset_name == "trackml-large" else ["data21149_s0.pt"]
    ...
    def download(self):
        if self.dataset_name == "trackml-large":
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset_name", type=str, default="trackml-large")
